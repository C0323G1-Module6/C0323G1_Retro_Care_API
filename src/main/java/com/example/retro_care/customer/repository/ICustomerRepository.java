...
public interface ICustomerRepository extends JpaRepository<Customer, Long> {
    ...
    @Query(value = " SELECT c.id, c.code, c.name, c.birth_day as birthDay, c.address, c.phone_number as phoneNumber, c.note, " +
            ...
            "WHERE c.flag_deleted = 1 AND c.name LIKE :name AND c.code LIKE :code AND c.address LIKE :address AND c.phone_number LIKE :phoneNumber AND " +
    ...
    Page<ICustomerDto> findAllCustomer(@Param(value = "name") String name, @Param(value = "code") String code, @Param(value = "address") String address, @Param(value = "phoneNumber") String phoneNumber, @Param(value = "groupValue") String groupValue, @Param(value = "sortItem") String sortItem, Pageable pageable);
